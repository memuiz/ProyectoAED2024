...
ggplot(datos, aes(x = factor(Periodo), y = Gasto_total, fill = CCAA)) +
  geom_col(position = "stack") +  # "stack" para apiladas
  labs(
...
```
Por otro lado, en esta gráfica observamos como Alemania es el país con mayor gasto en todas las comunidades autónomas. Seguido de cerca por Francia, nuestros vecinos
...
ggplot(datos, aes(x = Duracion_media, y = Gasto_medio_persona, size = Gasto_total, color = CCAA)) +
  ...
  theme_minimal()
```
```{r fig.width=5, fig.height=2.5, fig.cap= "Distribución del Gasto Total por País a lo largo de los Años.\\label{fig:ridgelineplot}", message = FALSE}
