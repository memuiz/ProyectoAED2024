...
Para comparar, para cada país, cuánto gastan en cada CCAA:
...
ggplot(datos %>% filter(Pais != "Otros"), 
  ...
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Gasto total por país y CCAA (sin 'otros')",
    x = "CCAA",
    y = "Gasto total"
  ) +
  theme_minimal()
...
Para observar si existe alguna relación entre la duración media de un viaje y el gasto medio por persona, a la vez que se detecta qué CCAA tienen un gasto total más elevado
...
Para mostrar la distribución de gasto_total por país y por año vamos a usar un ridgeline plot,. Este tipo de gráfico muestra la forma (densidad) de la distribución para cada combinación de año y país, apilando las curvas unas encima de otras. 
...
ggplot(datos %>% filter(Pais != "Otros"), aes(x = Gasto_total, 
  ...
  scale_fill_brewer(palette = "Set3") + 
  labs(
    title = "Distribución del Gasto Total por País a lo largo de los Años Sin Otros",
    subtitle = "Ridgeline plot para una visualización elegante de densidades",
    x = "Gasto Total",
    y = "Año",
    fill = "País"
...
```
Para visualizar la época de covid vamos a probar a graficarla de una manera distinta, nos quedaremos con la más adecuada.
...
ggplot(datos_summary, aes(x = factor(Periodo), y = Gasto_total_anual, fill = highlight)) +
  ...
  scale_fill_manual(values = c("2020 (COVID)" = "#e63946",  # rojo elegante
                               "Otros años"   = "#457b9d")) # azul
  labs(
    title = "Evolución del Gasto Total (antes, durante y después de 2020)",
