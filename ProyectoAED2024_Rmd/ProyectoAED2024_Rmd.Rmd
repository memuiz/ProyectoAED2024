...
ggplot(datos %>% filter(Pais != "Otros"), 
  ...
  labs(
    title = "Gasto total por país en las distintas Comunidades Autónomas (CCAA)",
    x = "CCAA",
    y = "Gasto total"
  ) +
  theme_minimal() +
...
```
Cada curva representa la densidad de la distribución del gasto total, con áreas más altas indicando rangos de gasto donde se concentra la mayor parte de los turistas. Se observa un aumento progresivo del gasto en los años más recientes, especialmente en 2021 y 2022, donde las curvas se desplazan hacia valores más altos. Esto es debido a un incremento en el gasto turístico promedio, posiblemente impulsado por la recuperación del sector tras la pandemia. Los turistas de los Países Nórdicos y, en menor medida, del Reino Unido, destacan con distribuciones que se extienden hacia valores de gasto más altos en comparación con otros países como Italia y Francia, que muestran concentraciones en valores más bajos


```{r fig.width=5.5, fig.height=2.5,  fig.cap="Distribución de gasto medio por persona por CCAA. .\\label{fig:cajas}"}
...
```
Observamos que destinos insulares como las Islas Baleares y Canarias lideran en términos de gasto, mientras que comunidades como Cataluña, Comunitat Valenciana y otras CCAA tienen gastos más modestos. Esto puede estar influenciado por el perfil de turistas que visitan estas regiones, la duración de la estancia y las actividades disponibles.
