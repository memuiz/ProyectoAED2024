...
ggplot(datos_totales, aes(x = Periodo, y = Gasto_total, color = CCAA, group = 1)) +
  geom_point() +
...
ggplot(datos_totales[datos_totales$Periodo == 2023, ], aes(x = reorder(Pais, -Gasto_total), y = Gasto_total)) +
...
ggplot(datos_totales, aes(x = Pais, y = Duracion_media)) + 
...
covid <- datos_totales[datos_totales$Periodo == 2020, ]
no_covid <- datos_totales[datos_totales$Periodo != 2020, ]
...
datos_totales %>% ggplot(aes(x = factor(Periodo), y = Gasto_total, fill = (Periodo == 2020))) +
...
```{r fig.height= 6}
ggplot(datos_totales, aes(x = CCAA, y = Gasto_total)) +
  geom_boxplot(outlier.colour = "red", fill = "lightgreen") +
  labs(title = "Distribución del Gasto por Comunidad Autónoma",
...
#
datos_total_por_residencia %>% filter(Pais != "Total") %>% ggplot(aes(x =Periodo, y = Gasto_total)) + geom_point(aes(color = Pais), size = 2) + geom_line() + facet_wrap(.~Pais, ncol = 2, scales = "free") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  labs(title = "Distribución del Gasto total por Pais para cada año")
```
