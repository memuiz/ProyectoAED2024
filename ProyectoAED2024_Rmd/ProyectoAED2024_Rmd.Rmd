...
ggplot(datos %>% filter(Pais != "Otros"), 
  ...
  labs(
    title = "Media gasto total por país en las distintas CCAA",
    x = "CCAA",
    y = "Media Gasto total"
  ) +
  theme_minimal() +
...
Otra forma de visualizar estos datos puede ser mostrando los países en una disposición geográfica dentro de un mapa, donde también se puede comparar la reducción del gasto Total a causa de la pandemia entre 2020 y 2021
...
```{r , fig.width=7, fig.height=4,  fig.cap="Distribución de gasto Total por país a lo largo de los años. .\\label{fig:geografGastoTotal}"}
geographic_map_gasto_total <-  ggplot(datos_total_residencia_viz)  + 
   geom_sf(aes(geometry= geometry,      
          fill=Gasto_total), 
          linewidth=0.5) + coord_sf(expand = FALSE) + facet_wrap(~Periodo) +
   theme(
  ...
  ylim(c(1380000, 5500000)) +
    labs(
    title = "Distribución de gasto Total por países
     extranjeros en España (2016-2023)"
  )
  
  geographic_map_gasto_total
```
...
```{r, warning=FALSE, fig.width=7, fig.height=4, fig.cap="Distribución de gasto medio por persona por CCAA en mapa geográfico por años. .\\label{fig:GeografGastoMedio}"}
datos_totales_por_CCAA <- subset(datos_totales,datos_totales$Pais == "Total" & datos_totales$CCAA != "Total")
datos_totales_por_CCAA$CCAA <- as.factor(datos_totales_por_CCAA$CCAA) 
datos_totales_por_CCAA$CCAA <- droplevels(datos_totales_por_CCAA$CCAA)
# Cambiamos de nombre los niveles del factor
...
levels(datos_totales_por_CCAA$CCAA) <- c("Andalucía", "Illes Balears","Canarias", "Cataluña","Comunitat Valenciana", "Comunidad de Madrid", "Otras CCAA")
spain_Comunities_Total <- spain_CCAAs %>% filter(LEVL_CODE == 2 & NUTS_NAME %in% unique(datos_totales_por_CCAA$CCAA))
spain_Comunities_Total <- spain_Comunities_Total %>% inner_join(datos_totales_por_CCAA, by=join_by(NUTS_NAME == CCAA))

spain_Other_Comunities_Total <- spain_CCAAs %>% filter(LEVL_CODE == 2 & !NUTS_NAME %in% unique(datos_totales_por_CCAA$CCAA))
spain_Other_Comunities_Total <- cbind(spain_Other_Comunities_Total, CCAA = c("Otras CCAA"))
spain_Other_Comunities_Total <- spain_Other_Comunities_Total %>% inner_join(datos_totales_por_CCAA, by=join_by(CCAA == CCAA))

spain_Comunities_Total %>% 
  ggplot(aes(geometry = geometry)) +
  geom_sf(
    data = spain_Comunities_Total,
    aes(fill = Gasto_medio_persona),
    color = "black",
    linewidth = 0.5
  ) +
  geom_sf(
    data = spain_Other_Comunities_Total,
    aes(fill = Gasto_medio_persona),
    color = "black",
    linewidth = 0.5
  )+ coord_sf(expand = FALSE)  + facet_wrap(~Periodo) + ggtitle(label = "Gasto Medio por persona en las distintas comunidades", subtitle = "desde 2016 hasta 2023") + 
   theme(
    axis.text.x = element_blank(),  
    axis.text.y = element_blank(),
    axis.ticks = element_blank()) 
...
Comparando las distribuciones anteriores respecto del gasto medio por persona podemos observar como se comparten las afirmaciones para un mapa geográfico agrupado por los distintos años disponibles. Canarias y Baleares suelen representar el mismo tono de color (lo que significa que los valores para cada año son similares) mientras que en el resto de comunidades muestran una variación mayor (la distribución de valores es más variable). 

A partir de esta variable se puede observar como el periodo de covid no afecta prácticamente (Las personas se siguen gastando lo mismo de media aunque no viajen tantas), sin embargo, durante los periodos de 2022 y 2023 se puede observar un incremento en el gasto lo cual es consecuencia del aumento de los precios en España.
...
geographic_map_gasto_medio_diario <-  ggplot(datos_total_residencia_viz)  + 
  ...
  geographic_map_gasto_medio_diario
