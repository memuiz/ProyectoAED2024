...
# lapply(datos,class)
```
Comprobamos si al realizar la transformación se han introducido datos NA y
```{r, include = FALSE}
any(is.na(datos))
```
...
```{r, fig.cap="Gráficos de dispersión y correlaciones entre variables cuantitativas", fig.label="fig:correlaciones"}
library(GGally)  # Librería para ggpairs
ggpairs(datos[4:7],label = FALSE)+theme(axis.text = element_blank()) # Quita los números de los ejes
...
ggplot(data = datos, aes(x=Gasto_medio_persona, y =Duracion_media, color = CCAA)) + geom_point() + facet_wrap(~ Pais, labeller = label_wrap_gen(width = 15)) +
  labs(title = "Relación entre gasto medio por persona y duración media por país de residencia",
...
Se observa una relación aproximadamente lineal para la mayoría de países, donde se distinguen claramente la tendendica de duración de los viajes en las distintas comunidades autónomas, casi siempre mayor en la Comunitat Valenciana (salvo en Italia, probablemente por cercanía), excepto en Otros. Debido a que esta categoría engloba a el resto de países del mundo, es más díficil encontara relaciones.
...
ggplot(data = datos, aes(x=Gasto_medio_persona, y =Gasto_medio_diario_persona, color = Pais)) + geom_point() + facet_wrap(~ CCAA, labeller = label_wrap_gen(width = 15)) +
  labs(title = "Relación entre gasto medio por persona y gasto medio diario por comunidad autónoma",
       x = "Gasto medio por persona (euros)",
...
ggplot(data = datos, aes(x=Gasto_medio_diario_persona, y =Duracion_media, color = CCAA)) + geom_point() + facet_wrap(~ Pais, labeller = label_wrap_gen(width = 15)) +
  labs(title = "Relación entre gasto medio por persona y duración media por país de residencia",
       x = "Gasto medio por persona (euros)",
       y = "Duración media de los viajes (días)") + theme_minimal() +
  geom_text(
...
```
Ahora podemos observar de manera clara que a mayor duración de los viajes se produce un menor gasto medio por persona.